...
pub struct GradesDeviations {
    ...
    pub hole_position_letters: Vec<String>,
...
impl GradesDeviations {
    pub fn default() -> Self {
        ...
        let shaft_position_letters = DEVIATION_MAP
            .iter()
            .map(|deviation| deviation.to_string())
            .collect::<Vec<_>>();

        let hole_position_letters = DEVIATION_MAP
            .iter()
        ...
        Self {
            it_numbers,
            hole_position_letters,
            shaft_position_letters,
        }
    }
}
...
impl IsoFit {
    ...
    pub fn convert(&self, size: f64) -> Option<Tolerance> {
        ...
        let grade_id = GRADE_MAP.iter().position(|&g| g == &self.grade)? + 1; // +1 to ignore column
        ...
        let deviation = if &self.deviation.to_uppercase() == "H" {
            0.0
        } else if &self.deviation.to_uppercase() == "JS" {
            -tolerance / 2.0
        } else {
